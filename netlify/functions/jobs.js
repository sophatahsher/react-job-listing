...
exports.handler = async function (event, context) {
    if (event.httpMethod === 'GET') {
      ...
      const limit = event.queryStringParameters._limit
        ? parseInt(event.queryStringParameters._limit, 10)
        : jobs.length;

      return {
        statusCode: 200,
        body: JSON.stringify(jobs.slice(0, limit)),
      };
    } else if (event.httpMethod === 'POST') {
      const data = JSON.parse(event.body);
      const jobsRef = db.collection('jobs');
      const result = await jobsRef.add(data);
